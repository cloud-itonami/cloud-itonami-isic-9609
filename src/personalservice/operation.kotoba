(ns personalservice.operation
  "OperationActor -- one personal-service operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (PersonalServiceOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Personal Service Governor
  ...
  One graph run = one personal-service operation (intake -> advise ->
  ...
  `:actuation/finalize-referral` ALWAYS reaches this node when the
  governor is clean -- see `personalservice.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [personalservice.personalserviceadvisor :as personalserviceadvisor]
            [personalservice.governor :as governor]
...
(defn build
  ...
  `personalservice.store/Store`).
  opts:
    :advisor      -- a `personalservice.personalserviceadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (personalserviceadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; PersonalServiceOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (personalserviceadvisor/-advise advisor store request)]
            {:proposal p :audit [(personalserviceadvisor/trace request p)]})))

      ;; Personal Service Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
